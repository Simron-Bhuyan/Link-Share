...
const Sidebar = ({ user, closeToggle }) => {
  ...
  return (
    <div className="flex flex-col justify-between bg-white h-full min-w-210 hide-scrollbar overflow-y-scroll">
...
{categories.slice(0,categories.length-1).map((category)=>(
  ...
  {category.name}
      ...
      {user && (
        <Link to={`user-profile/${user._id}`} className="flex my-5 mb-3 gap-2 p-2 items-center bg-white rounded-lg shadow-lg mx-5">
